...
def _scores_after_first_iteration(mc: MonteCarloLCA) -> np.ndarray:
    """Scores from iterations 1..n (iteration 0 reuses baseline matrices)."""
    return mc.results[1:, 0, 0]
...
def test_mc_single_source_produces_spread(mc_project, includes):
    ...
    assert np.std(_scores_after_first_iteration(mc)) > 0
...
def test_mc_reproducible_with_same_seed(mc_project):
    ...
    first = _scores_after_first_iteration(_run_mc(mc_project, **kwargs))
    second = _scores_after_first_iteration(_run_mc(mc_project, **kwargs))
