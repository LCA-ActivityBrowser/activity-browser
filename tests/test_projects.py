...
def test_new_project(qtbot, mock, ab_app):
    ...
    mock.patch.object(
        Controller, 'get_new_project_name_diaglogue', return_value='pytest_project_del'
    )
    qtbot.mouseClick(
...
def test_delete_project(qtbot, mock, ab_app):
    qtbot.waitForWindowShown(ab_app.main_window)
    assert bw.projects.current == 'pytest_project_del'
    mock.patch.object(Controller, 'confirm_project_deletion_dialogue', return_value=True)
    qtbot.mouseClick(
