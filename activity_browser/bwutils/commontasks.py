...
def switch_brightway2_dir(dirpath):
    bw_base = projects._base_data_dir
    if dirpath == bw_base:
    ...
    try:
        ...
        log.info('Loaded brightway2 data directory: {}'.format(projects._base_data_dir))
        return True
...
def cleanup_deleted_bw_projects() -> None:
    ...
    """
    n_dir = projects.purge_deleted_directories()
...
def is_technosphere_db(db_name: str) -> bool:
    ...
    act = DatabaseChooser(db_name).random()
    return act is None or act.get("type", "process") == "process"
...
def get_exchanges_from_a_list_of_activities(activities: list, as_keys: bool = False) -> list:
    ...
    if as_keys:
        activities = [get_activity(key) for key in activities]
    exchanges = []
