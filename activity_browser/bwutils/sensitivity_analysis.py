...
def get_exchanges(lca, indices, biosphere=False, only_uncertain=True):
    ...
    for i in indices:
        if biosphere:
            from_act = get_activity(lca.biosphere_dict_rev[i[0]])
        else:  # technosphere
            from_act = get_activity(lca.activity_dict_rev[i[0]])
        to_act = get_activity(lca.activity_dict_rev[i[1]])
...
def get_exchanges_dataframe(exchanges, indices, biosphere=False):
    ...
    for exc, i in zip(exchanges, indices):
        from_act = get_activity(exc.get('input'))
        to_act = get_activity(exc.get('output'))
...
def get_CF_dataframe(lca, only_uncertain_CFs=True):
    ...
    for params_index, row in enumerate(lca.cf_params):
        if only_uncertain_CFs and row['uncertainty_type'] <= 1:
            continue
        cf_index = row['row']
        bio_act = get_activity(lca.biosphere_dict_rev[cf_index])
...
class GlobalSensitivityAnalysis(object):
    ...
    def perform_GSA(self, act_number=0, method_number=0,
        ...
        try:
            ...
            self.activity = get_activity(self.mc.rev_activity_index[act_number])
        ...
        log.info('-- GSA --\n Project:', projects.current, 'CS:', self.mc.cs_name,
