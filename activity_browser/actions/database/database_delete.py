...
from activity_browser.brightway.bw2data.backends.proxies import ExchangeDataset, Exchanges
...
class DatabaseDelete(ABAction):
    ...
    def run(db_name: str):
        ...
        excs._args = [ExchangeDataset.input_database == db_name, ExchangeDataset.output_database != db_name]
        n_upstream_excs = len(excs)

        # construct warning text
        text = f"Are you sure you want to delete database '{db_name}'?"
        if n_records: text += f" It contains {n_records} activities"
        if n_upstream_excs: text += f" and {n_upstream_excs} exchanges to other databases"

        # ask the user for confirmation
        QtWidgets.QApplication.restoreOverrideCursor()
        response = QtWidgets.QMessageBox.question(application.main_window, "Delete database?", text)
